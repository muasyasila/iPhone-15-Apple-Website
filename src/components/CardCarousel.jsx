...
const CardCarousel = () => {
  ...
  return (
    <div className="container mx-auto p-4 bg-zinc">
        ...
        <h1 id="title" className="section-heading mb-8 mt-8 ml-4">The latest. Take a look at what else is new.</h1>
       <Slider {...settings}>
        {cards.map((card) => (
          <div key={card.id} className="p-4" style={{ width: 'calc(100% / 4)' }}>
            <div className="mt-8 relative bg-cover bg-center h-96 rounded-3xl shadow-lg overflow-hidden" style={{ backgroundImage: `url(${card.image})` }}>
